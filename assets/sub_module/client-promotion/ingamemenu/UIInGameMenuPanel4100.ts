 ...
 * @20231103 fix
 * @20231205 remove floatingButton
 */
@ccclass( 'UIInGameMenuPanel4100' )
export class UIInGameMenuPanel4100 extends Component {
    /** 本身是浮動式按鈕 */
    ...
    // @property( { displayName: 'nodePromotionTipWindow', type: Node, tooltip: 'node of PromotionTipWindow', group: { name: "setting", id: '5' } } )
    // public nodePromotionTipWindow !: Node;
    // private scriptPromotionTipWindow !: promotionTipWindow;
    ...
    public static Instance: UIInGameMenuPanel4100;

    protected onLoad(): void {
        UIInGameMenuPanel4100.Instance = this;
    }
    ...
    start () {
        ...
        if (this.buttonPromotion?.node) igmButtonUtils.setNodeEventOnHover( this.buttonPromotion.node );
        if (this.buttonJackpot?.node)   igmButtonUtils.setNodeEventOnHover( this.buttonJackpot.node );

        this.resetPosition();

        // this.scriptPromotionTipWindow = this.nodePromotionTipWindow.getComponent<promotionTipWindow>( promotionTipWindow )!;
    ...
    public onButtonInGameMenuClick (): void {
        ...
        let element = document.getElementsByClassName('igm_close')?.[0];
    ...
    processPromotionBriefResponse ( promotionBriefResponse ) {
    ...
    async afterGameReadyToSpin () {
        var promotionResponse = await this.fetchPromotionBrief();
        promotionResponse = this.processPromotionBriefResponse( promotionResponse );
    ...
    public createJackpotItem (): void {
        ...
        //PromotionHint 
        let key: igmOrientation = ( Viewport.instance.getCurrentOrientation() === Orientation.PORTRAIT ) ? igmOrientation.PORTRAIT : igmOrientation.LADNSCAPE;
        let iconPosition: Vec3 = this.config.JackpotHintSpriteIcon.get( key );
    ...
    public isPortrait (): boolean {
        return ( Viewport.instance.getCurrentOrientation() === Orientation.PORTRAIT );
    }
    ...
    public setSoundMuted ( set: boolean ) {
        this.soundMuted = set;
        if ( this.floatingBoardCurrent ) {
            this.floatingBoardCurrent.setSoundMuted( set );
        }
    }
}
